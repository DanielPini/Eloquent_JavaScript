...
export const goalOrientedRobot = ({ place, parcels }, memory = []) => {
    if (memory.length == 0) {
...
export const carefulRobot = ({ place, parcels }, memory) => {
...
export const smartRobot = ({ place, parcels }, memory = []) => {
    if (memory.length === 0) {
        // 1. Map parcels to PotentialRoute objects
        const potentialRoutes = parcels.map((parcel) => {
        ...
        const bestBranch = potentialRoutes.reduce((prev, curr) => curr.score < prev.score ? curr : prev);
