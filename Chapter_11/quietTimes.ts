...
async function generateFakeTimestamps(): Promise<string> {
...
const activityTable = async (day: number) => {
  let dataArray: number[] = new Array(24).fill(0);
...
// Without async / await
function generateFakeTimestamps2(): Promise<string> {
  return new Promise((res) => {
    let stamps = "";
    for (let i = 0; i < 500; i++) {
      const timestamp = Math.floor(Math.random() * 9936000000 + 1363179600000);
...
const activityTable2 = (day: number) => {
