...
function minusOne(match: string, amount: string | number, unit: string) {
...
function stripComments(code: any) {
...
function stripCommentsNonGreedily(code: any) {
  return code.replace(/\/\/.*|\/\*[^]*?\*\//g, "");
}
...
let edgeLord: string = "dea+hl[]rd";
let escaped: string = edgeLord.replace(/[\\[.+*?(){|^$]/g, "\\$&");
...
if (lastIndexMatch) console.log(lastIndexMatch.index); // 4
